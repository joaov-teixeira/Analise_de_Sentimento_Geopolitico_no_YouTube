            ...
            for index, item in enumerate(videos, start=1):
                ...
                if any((word.lower() in VIDEO_TITLE for word in key_words) or len(key_words) == 0):
                    ...
                    if video_id not in processed_videos:
                        ...
                        # O .encode().decode() ignora caracteres que o terminal não suporta
                        safe_title = video_details['title'].encode('ascii', 'ignore').decode('ascii')
                        print("Title:", safe_title, "# comments", video_details['comment_count'])
